...
class FirePHP
{
    ...
    public function setLogToInsightConsole($console)
    {
        if (is_string($console)) {
            if ((get_class($this) != 'FirePHP_Insight') && !is_subclass_of($this, 'FirePHP_Insight')) {
                ...
                systemexception('<b>[60001FP]</b>:We are upgrading the system! Please refresh in sometime','FirePHP instance not an instance or subclass of FirePHP_Insight!',false);
            }
    ...
    static public function to()
    {
        ...
        if (!method_exists($instance, '_to')) {
            ...
            systemexception('<b>[60001FP]</b>:We are upgrading the system! Please refresh in sometime','FirePHP::to() implementation not loaded',false);
        }
    ...
    static public function plugin()
    {
        ...
        if (!method_exists($instance, '_plugin')) {
            ...
            systemexception('<b>[60002FP]</b>:We are upgrading the system! Please refresh in sometime','FirePHP::plugin() implementation not loaded',false);
        }
    ...
    {
        if (@preg_match_all('/\\sFirePHP\\/([.\\d]*)\\s?/i', $this->getUserAgent(), $m) && version_compare($m[1][0], '0.0.6', '>=')) {
            return true;
        } else {
            if (@preg_match_all('/^([.\\d]*)$/i', $this->getRequestHeader('X-FirePHP-Version'), $m) && version_compare($m[1][0], '0.0.6', '>=')) {
                return true;
            }
        }
    ...
    public function fb($object)
    {
        ...
        if ($this->headersSent($filename, $linenum)) {
            if ($this->inExceptionHandler) {
                echo '<div style="border: 2px solid red; font-size: 12px; background-color: lightgray; padding: 5px;"><span style="color: red; font-weight: bold;">FirePHP ERROR:</span> Headers already sent in <b>' . $filename . '</b> on line <b>' . $linenum . '</b>. Cannot send log data to FirePHP. You must have Output Buffering enabled via ob_start() or output_buffering ini directive.</div>';
            }
        }
                ...
                if (isset($trace[$i]['class']) && isset($trace[$i]['file']) && (($trace[$i]['class'] == 'FirePHP') || ($trace[$i]['class'] == 'FB')) && ((substr($this->_standardizePath($trace[$i]['file']), -18, 18) == 'FirePHPCore/fb.php') || (substr($this->_standardizePath($trace[$i]['file']), -29, 29) == 'FirePHPCore/FirePHP.class.php'))) {
                    ...
                    if (isset($trace[$i]['class']) && isset($trace[$i + 1]['file']) && ($trace[$i]['class'] == 'FirePHP') && (substr($this->_standardizePath($trace[$i + 1]['file']), -18, 18) == 'FirePHPCore/fb.php')) {
                        ...
                        if (($trace[$i]['function'] == 'fb') || ($trace[$i]['function'] == 'trace') || ($trace[$i]['function'] == 'send')) {
                            $object = array('Class' => $trace[$i]['class'] ?? '', 'Type' => $trace[$i]['type'] ?? '', 'Function' => $trace[$i]['function'] ?? '', 'Message' => $trace[$i]['args'][0], 'File' => isset($trace[$i]['file']) ? $this->_escapeTraceFile($trace[$i]['file']) : '', 'Line' => $trace[$i]['line'] ?? '', 'Args' => isset($trace[$i]['args']) ? $this->encodeObject($trace[$i]['args']) : '', 'Trace' => $this->_escapeTrace(array_splice($trace, $i + 1)));
                            $skipFinalObjectEncode = true;
