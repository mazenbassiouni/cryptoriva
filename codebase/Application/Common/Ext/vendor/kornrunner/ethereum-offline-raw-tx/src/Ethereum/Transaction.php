...
class Transaction {
    ...
    public function getRaw(string $privateKey, int $chainId = 0): string {
        if ($chainId < 0) {
		   ...
		   dblog('Dex Issue', 'in File :'.__FILE__.' on line '.__LINE__.' : ChainID must be positive');
		   return false;
        }
        ...
        if (strlen($privateKey) != 64) {
            dblog('Dex Issue',' in File :'.__FILE__.' on line '.__LINE__.'Incorrect private key');
			//mlog('dex_issue','in File :'.__FILE__.' on line '.__LINE__.' -> Incorrect private key');
			return false;
        }
