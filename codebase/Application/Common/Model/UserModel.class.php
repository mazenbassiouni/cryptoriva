    ...
    public function check_update()
    {
        ...
        if (!$check_update_user) {
            ...
            if (!in_array('alipay', $User_DbFields)) {
                M()->execute('ALTER TABLE `codono_user` ADD COLUMN `alipay` VARCHAR(200) NULL  COMMENT \'Alipay\' AFTER `status`;');
            }

            if (!in_array('email', $User_DbFields)) {
                M()->execute('ALTER TABLE `codono_user` ADD COLUMN `email` VARCHAR(200) NULL  COMMENT \'mailbox\' AFTER `status`;');
            }
