...
function maxbuy(i){
	...
	var max = formatfloat(FINANCE.data['usd_balance']/parseFloat($('#buy-price').val()), 3, 0);
...
AJAX.finance_cb = function(){
	if(!parseInt($.cookie('kline'))) {
		...
		'<tr><th>Available'+COIN[CONF.coin]+'</th><td><font>'+FINANCE.data[CONF.coin + '_balance']+'</font></td></tr>'+
		'<tr><th>Freeze'+COIN[CONF.coin]+'</th><td><span>'+FINANCE.data[CONF.coin + '_lock']+'</span></td></tr>'+
		'<tr><th>USD</th><td class="zc-c2c">loading...</td></tr>'+
			'<tr><th>Available USD</th><td><font>$'+formatfloat(FINANCE.data['usd_balance'], 2, 0)+'</font></td></tr>'+
		'<tr><th>Freeze USD</th><td><font style="color:#e55600;">$'+formatfloat(FINANCE.data['usd_lock'], 2, 0)+'</font></td></tr></table></div>');
		$('.zcxx').show();
	}else{
		$('#uinfo').html('<div style="width:100%;height:46px;background:#3d3d3d;">' +
		'<div class="autobox">' +
		'<ul class="available-info">' +
		'<li style="color:#80C000;">Available'+ COIN[CONF.coin] +'：<span style="color:#80C000;">'+ FINANCE.data[CONF.coin + '_balance'] + '</span></li>' +
		'<li style="color:#f95919;">freeze'+ COIN[CONF.coin] +'：<span style="color:#f95919;">'+ FINANCE.data[CONF.coin + '_lock'] +'</span></li>' +
		'<li>USD:<span class="zc-c2c">loading...</span></li><li>Available USD:<span>$'+ formatfloat(FINANCE.data['usd_balance'], 2, 0) +'</span></li></ul></div></div>');
	}
